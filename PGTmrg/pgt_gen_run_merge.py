...
from pgt_naive_txtvis_merge import get_naive_model
from pgt_gen_merge import gen_pgt
...
# In[12]:
    ...
    for exp in exp_name:
        exps_rslts[exp] = {
                           'baseline':[], # baseline
                           'onlypgt':[], # pgt only
                           'pgt_gt':[], # pgt + gt strategy
                           'finetune':[] # pgt -> gt strategy
                          }
    for iter_count in range(run_count):
        ...
        for gt_set, exp in zip(gt_sets, exp_name):
            ...
            gen_pgt(gt_set, 'GT_comb_fold_0_val.csv', exp, var_save_name)
        ...
        # generated pseudo-groundtruth save path
        ...
        # save path for the model trained with only pseudo-groundtruth
        pgt_model_paths = ['cnn_160gt_forGTGen_w_pgt_160trainless_original_'+var_save_name+'.pt',
                           'cnn_160gt_forGTGen_w_pgt_160trainless_reduct_f2_'+var_save_name+'.pt',
                           'cnn_160gt_forGTGen_w_pgt_160trainless_reduct_f4_'+var_save_name+'.pt',
                           'cnn_160gt_forGTGen_w_pgt_160trainless_reduct_f8_'+var_save_name+'.pt',
                           'cnn_160gt_forGTGen_w_pgt_160trainless_enlarge_f2_'+var_save_name+'.pt',
                           'cnn_160gt_forGTGen_w_pgt_160trainless_enlarge_f4_'+var_save_name+'.pt',
                           'cnn_160gt_forGTGen_w_pgt_160trainless_enlarge_f8_'+var_save_name+'.pt']
        
        # manual groundtruth (same as gt_sets), can use gt_sets directly, here just for convenient readability
        fine_tune_sets = ['GT_comb_fold_0_train.csv',
                          'GT_comb_fold_0_train_reduct_f2.csv', 
                          'GT_comb_fold_0_train_reduct_f4.csv',
                          'GT_comb_fold_0_train_reduct_f8.csv',
                          'GT_comb_fold_0_train_enlarge_f2.csv', 
        ...
        # save path for the model trained with pgt -> gt strategy
        fine_tune_model_paths = ['cnn_160gt_forGTGen_w_pgt_160trainless_tune_160train_original_'+var_save_name+'.pt',
                                 'cnn_160gt_forGTGen_w_pgt_160trainless_tune_160train_reduct_f2_'+var_save_name+'.pt',
                                 'cnn_160gt_forGTGen_w_pgt_160trainless_tune_160train_reduct_f4_'+var_save_name+'.pt',
                                 'cnn_160gt_forGTGen_w_pgt_160trainless_tune_160train_reduct_f8_'+var_save_name+'.pt',
                                 'cnn_160gt_forGTGen_w_pgt_160trainless_tune_160train_enlarge_f2_'+var_save_name+'.pt',
                                 'cnn_160gt_forGTGen_w_pgt_160trainless_tune_160train_enlarge_f4_'+var_save_name+'.pt',
                                 'cnn_160gt_forGTGen_w_pgt_160trainless_tune_160train_enlarge_f8_'+var_save_name+'.pt']

        for pgt_set, pgt_model, tune_set, tune_model, exp in zip(pgt_sets, 
                                                                 pgt_model_paths, 
                                                                 fine_tune_sets, 
                                                                 fine_tune_model_paths, 
                                                                 exp_name):
            print(pgt_set, pgt_model, tune_set, tune_model, exp)
            run_train_pgt(pgt_set, 
                          'GT_comb_fold_0_val.csv',
                          'images',
                          pgt_model,
                          disp
                         )
            print(exp, 'onlypgt', 'train')
            prec, recl, f1, cur_acc = run_test('pseudoGT_test_1700.csv',
                                               'images',
                                               pgt_model,
                                               disp
                                              )
            print(exp, 'onlypgt', 'test')
            exps_rslts[exp]['onlypgt'].append([prec, recl, f1, cur_acc])

            run_fine_tune(pgt_model,
                          tune_set,
                          'GT_comb_fold_0_val.csv',
                          'images',
                          tune_model,
            ...
            print(exp, 'finetune', 'train')
            prec, recl, f1, cur_acc = run_test('pseudoGT_test_1700.csv',
                                               'images',
                                               tune_model,
                                               disp
                                              )
            print(exp, 'finetune', 'test')
            exps_rslts[exp]['finetune'].append([prec, recl, f1, cur_acc])
                    
        # generated pseudo-groundtruth save path (same as the above, here just for convenient readability)
        pgt_sets = ['pseudoGT_unknow_14000_pgt_no160train_original_'+var_save_name+'.csv',
                    'pseudoGT_unknow_14000_pgt_no160train_reduct_f2_'+var_save_name+'.csv',
                    'pseudoGT_unknow_14000_pgt_no160train_reduct_f4_'+var_save_name+'.csv', 
                    'pseudoGT_unknow_14000_pgt_no160train_reduct_f8_'+var_save_name+'.csv',
                    ...
                    'pseudoGT_unknow_14000_pgt_no160train_enlarge_f8_'+var_save_name+'.csv']
        # save path for the model trained with pgt + gt strategy
        pgt_model_paths = ['cnn_160gt_forGTGen_w_pgt_160train_original_'+var_save_name+'.pt',
                           'cnn_160gt_forGTGen_w_pgt_160train_reduct_f2_'+var_save_name+'.pt',
                           'cnn_160gt_forGTGen_w_pgt_160train_reduct_f4_'+var_save_name+'.pt',
                           'cnn_160gt_forGTGen_w_pgt_160train_reduct_f8_'+var_save_name+'.pt',
                           'cnn_160gt_forGTGen_w_pgt_160train_enlarge_f2_'+var_save_name+'.pt',
                           'cnn_160gt_forGTGen_w_pgt_160train_enlarge_f4_'+var_save_name+'.pt',
                           'cnn_160gt_forGTGen_w_pgt_160train_enlarge_f8_'+var_save_name+'.pt']
        # save path for the combined pseudo-groundtruth and manual groundtruth, the combined GT is shuffled
        comb_sets = ['pgt_14000_gt_160_original_'+var_save_name+'.csv',
                     'pgt_14000_gt_160_reduct_f2_'+var_save_name+'.csv',
                     'pgt_14000_gt_160_reduct_f4_'+var_save_name+'.csv',
                     'pgt_14000_gt_160_reduct_f8_'+var_save_name+'.csv',
                     'pgt_14000_gt_160_enlarge_f2_'+var_save_name+'.csv',
                     'pgt_14000_gt_160_enlarge_f4_'+var_save_name+'.csv',
                     'pgt_14000_gt_160_enlarge_f8_'+var_save_name+'.csv']

        for pgt_set, pgt_model, tune_set, comb_set, exp in zip(pgt_sets,
                                                               pgt_model_paths,
                                                               fine_tune_sets,
                                                               comb_sets,
                                                               exp_name):
            print(pgt_set, pgt_model, tune_set, comb_set, exp)
            combine_pgt_gt(pgt_set, tune_set, comb_set)
            run_train_pgt(comb_set, 
                          'GT_comb_fold_0_val.csv',
                          'images',
                          pgt_model,
                          disp
                         )
            print(exp, 'pgt_gt', 'train')
            prec, recl, f1, cur_acc = run_test('pseudoGT_test_1700.csv',
                                               'images',
                                               pgt_model,
                                               disp
                                              )
            print(exp, 'pgt_gt', 'test')
            exps_rslts[exp]['pgt_gt'].append([prec, recl, f1, cur_acc])
